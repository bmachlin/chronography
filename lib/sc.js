...
function callSpotify(url, data, callback, dataType) {
    ...
    if (AccessTokenSC === '') {
        ...
        return $.ajax({
            ...
            statusCode: {
                429: function(r, ts, jq) {
                    ...
                    let retryAfter = r.getAllResponseHeaders();
        ...
        return $.ajax({
            ...
            statusCode: {
                429: function(r, ts, jq) {
                    let retryAfter = r.getResponseHeader('Retry-After');
                    retryAfter = parseInt(retryAfter, 10);
    ...
    $.ajax({
        ...
        error: function(r) {
            ...
            if(r.responseText.includes('invalid_grant')) {
                getNewToken(localStorage.RefreshTokenSC, client, secret, callback);
            }
        },
        success: function(r) {
            console.log(r);
...
function isAccessExpired() {
    ...
    return localStorage.ExpireTimeSC === undefined || now > localStorage.ExpireTimeSC;
}
function accessTokenSet() {
    return AccessTokenSC != '' || (localStorage.AccessTokenSC != null && !isAccessExpired());
}
function resetAccess() {
    AccessTokenSC = '';
    ExpireTimeSC = new Date.getTime();
}
...
function fetchArtistMulti(artistIDs, callback) { 
    let idString = artistIDs[0];
    for(let i = 1; i < artistIDs.length && i < 100; i++) {
        idString += "," + artistIDs[i];
    }
    let url = 'https://api.spotify.com/v1/artists/?ids=' + idString;
    return callSpotify(url, {}, callback);
}
...
function fetchArtistAlbums(artistID, options, callback) {
    let url = "";
    if(options.hasOwnProperty('album_type')) {
        let typeString = "";
        for(let ty in options.album_type) {
            typeString += options.album_type[ty] + ",";
        }
        typeString = typeString.substring(0, typeString.length-1);
        url = 'https://api.spotify.com/v1/artists/' + artistID + '/albums' 
                                                + '?album_type=' + typeString;
    } else {
        url = 'https://api.spotify.com/v1/artists/' + artistID + '/albums';
    }
...
function fetchAlbumMulti(albumIDs, options, callback) { 
    let idString = albumIDs[0];
    for(let i = 1; i < albumIDs.length && i < 100; i++) {
        idString += "," + albumIDs[i];
    }
    let url = 'https://api.spotify.com/v1/albums/?ids=' + idString;
    return callSpotify(url, options, callback);
}
...
function fetchTrackMulti(trackIDs, options, callback) {
    let idString = trackIDs[0];
    for(let i = 1; i < trackIDs.length && i < 100; i++) {
        idString += "," + trackIDs[i];
    }
    let url = 'https://api.spotify.com/v1/tracks/?ids=' + idString;
    return callSpotify(url, options, callback);
}
...
function fetchAudioFeaturesMulti(trackIDs, callback) {
    ...
    let url = 'https://api.spotify.com/v1/audio-features/?ids=' + idString;
    return callSpotify(url, {}, callback);
}
...
function fetchSavedTracksContains(trackIDs, callback) {
    let idString = trackIDs[0];
    for(let i = 1; i < trackIDs.length && i < 50; i++) {
        idString += "," + trackIDs[i];
    }
    let url = 'https://api.spotify.com/v1/me/tracks/contains?ids=' + idString;
    return callSpotify(url, {}, callback);
}
...
function searchSpotify(query, options, callback) {
    let url = 'https://api.spotify.com/v1/search?query=' + encodeURIComponent(query);
